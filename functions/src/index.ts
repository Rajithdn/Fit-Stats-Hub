...
// ── Auth ──────────────────────────────────────────────────────────────────────
const JWT_SECRET = process.env.JWT_SECRET || "termfit-jwt-secret-2024-rajith";
...
app.post("/api/auth/register", async (req, res) => {
  try {
    ...
    if (!username?.trim() || !password || password.length < 4) { res.status(400).json({ error: "Username and password (min 4 chars) required" }); return; }
    ...
    await db.insert(userProfiles).values({ userId: user.id, name: name?.trim() ?? "", age: age ?? 25, gender: gender ?? "male", height: height ? String(height) : "170", weight: weight ? String(weight) : "70", targetWeight: targetWeight ? String(targetWeight) : "65", activityLevel: activityLevel ?? "Moderately Active", goal: goal ?? "Weight Loss" });
...
app.post("/api/auth/change-password", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (!user || !(await bcrypt.compare(currentPassword, user.passwordHash))) { res.status(401).json({ error: "Wrong current password" }); return; }
...
app.put("/api/profile", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [updated] = await db.update(userProfiles).set({ ...(name !== undefined && { name }), ...(age !== undefined && { age: Number(age) }), ...(gender !== undefined && { gender }), ...(height !== undefined && { height: String(height) }), ...(weight !== undefined && { weight: String(weight) }), ...(targetWeight !== undefined && { targetWeight: String(targetWeight) }), ...(activityLevel !== undefined && { activityLevel }), ...(goal !== undefined && { goal }), ...(dailyCalorieGoal !== undefined && { dailyCalorieGoal: Number(dailyCalorieGoal) }), ...(profilePhoto !== undefined && { profilePhoto }), ...(stepGoal !== undefined && { stepGoal: Number(stepGoal) }), ...(theme !== undefined && { theme }), updatedAt: new Date() }).where(eq(userProfiles.userId, req.userId!)).returning();
    res.json(updated);
  } catch { res.status(500).json({ error: "Failed" }); }
});
...
app.post("/api/food-logs", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [row] = await db.insert(foodLogs).values({ userId: req.userId!, date: date ?? new Date().toISOString().split("T")[0], name, calories: Number(calories), protein: String(protein), carbs: String(carbs), fat: String(fat), fiber: String(fiber ?? 0) }).returning();
    res.status(201).json(row);
  } catch { res.status(500).json({ error: "Failed" }); }
});
app.delete("/api/food-logs/:id", requireAuth, async (req: AuthRequest, res) => {
  try { await db.delete(foodLogs).where(and(eq(foodLogs.id, Number(req.params.id)), eq(foodLogs.userId, req.userId!))); res.json({ ok: true }); }
  catch { res.status(500).json({ error: "Failed" }); }
});

// Workout logs
app.get("/api/workout-logs", requireAuth, async (req: AuthRequest, res) => {
  try { res.json(await db.select().from(workoutLogs).where(eq(workoutLogs.userId, req.userId!)).orderBy(workoutLogs.createdAt)); }
  catch { res.status(500).json({ error: "Failed" }); }
});
app.post("/api/workout-logs", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { date, exercise, sets, reps, weight, unit, notes } = req.body;
    const [row] = await db.insert(workoutLogs).values({ userId: req.userId!, date: date ?? new Date().toISOString().split("T")[0], exercise, sets: Number(sets), reps: Number(reps), weight: String(weight ?? 0), unit: unit ?? "kg", notes: notes ?? "" }).returning();
    res.status(201).json(row);
  } catch { res.status(500).json({ error: "Failed" }); }
});
app.delete("/api/workout-logs/:id", requireAuth, async (req: AuthRequest, res) => {
  try { await db.delete(workoutLogs).where(and(eq(workoutLogs.id, Number(req.params.id)), eq(workoutLogs.userId, req.userId!))); res.json({ ok: true }); }
  catch { res.status(500).json({ error: "Failed" }); }
});

// Steps
app.get("/api/steps", requireAuth, async (req: AuthRequest, res) => {
  try { res.json(await db.select().from(stepEntries).where(eq(stepEntries.userId, req.userId!)).orderBy(stepEntries.date)); }
  catch { res.status(500).json({ error: "Failed" }); }
});
app.post("/api/steps", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (existing.length > 0) {
      const [row] = await db.update(stepEntries).set({ steps: Number(steps) }).where(and(eq(stepEntries.userId, req.userId!), eq(stepEntries.date, d))).returning();
      res.json(row);
    } else {
...
app.get("/api/measurements", requireAuth, async (req: AuthRequest, res) => {
  try { res.json(await db.select().from(measurements).where(eq(measurements.userId, req.userId!)).orderBy(measurements.date)); }
  catch { res.status(500).json({ error: "Failed" }); }
});
app.post("/api/measurements", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    const [row] = await db.insert(measurements).values({ userId: req.userId!, date: date ?? new Date().toISOString().split("T")[0], chest: String(chest ?? 0), waist: String(waist ?? 0), hips: String(hips ?? 0), leftArm: String(leftArm ?? 0), rightArm: String(rightArm ?? 0), leftThigh: String(leftThigh ?? 0), rightThigh: String(rightThigh ?? 0), shoulders: String(shoulders ?? 0), neck: String(neck ?? 0), notes: notes ?? "" }).returning();
    res.status(201).json(row);
  } catch { res.status(500).json({ error: "Failed" }); }
});
app.delete("/api/measurements/:id", requireAuth, async (req: AuthRequest, res) => {
  try { await db.delete(measurements).where(and(eq(measurements.id, Number(req.params.id)), eq(measurements.userId, req.userId!))); res.json({ ok: true }); }
  catch { res.status(500).json({ error: "Failed" }); }
});
...
app.post("/api/daily-logs", requireAuth, async (req: AuthRequest, res) => {
  try {
    ...
    if (existing.length > 0) {
      const [row] = await db.update(dailyLogs).set({ ...(water !== undefined && { water: String(water) }), ...(sleep !== undefined && { sleep: String(sleep) }) }).where(and(eq(dailyLogs.userId, req.userId!), eq(dailyLogs.date, d))).returning();
      res.json(row);
    } else {
...
export const api = onRequest({ region: "us-central1", timeoutSeconds: 60, memory: "512MiB" }, app);
